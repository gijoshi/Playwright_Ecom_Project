  ...
  const{loginPageNew, LoginPageNew} = require('../PageObjectsNew/LoginPageNew');
const { DashBoardNew } = require('../PageObjectsNew/DashBoardNew');
  require('../PageObjectsNew/DashBoardNew');
  ...
  test('Client Registration', async ({page})=>
    {
        ...
        await page.locator("[type='email']").fill("test"+randomNum+"@gmail.com");
        console.log("Following is the email used for registration: " + "test_"+randomNum+"@gmail.com");
  ...
  //Below code not completed, having few issues
   ...
   test.skip('Client Login UI Controls', async ({page})=>
    {
   ...
   const uName = "dzdz@gmail.com";
   const upwd = "Pass@123";
   const productName = 'ZARA COAT 3';


   const loginFlow1 = new LoginPageNew(page)
   await loginFlow1.openURL();
   await loginFlow1.loginflowPage(uName, upwd);
   ...
   expect(page.locator(".user__name [type='text']").first()).toHaveText(uName);
